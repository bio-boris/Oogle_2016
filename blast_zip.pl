...
my $list_length = scalar @list_of_organisms;
if($job_id > $list_length){
	die "Sorry, your job $job_id was not found, (organism listlength= $list_length)\n";
}
...
sub blast{
	...
	print ("time $blast_command > $o.time; mv $o.tmp $o");
	system("time $blast_command > $o.time; mv $o.tmp $o");
}
